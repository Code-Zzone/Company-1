const firebaseConfig = {
  ...
  async function getNextProductId(category) {
  ...
  async function addProduct() {
    ...
    if (!name || isNaN(price) || !image || !category) {
      showMessage("يرجى إدخال جميع البيانات بشكل صحيح، بما فيهم القسم.", "red");
      return;
    }
  
    const newId = await getNextProductId(category);
    ...
    db.ref("products/" + category + "/" + newId).set(newProduct, error => {
        if (error) {
...
function fetchCategories() {
    db.ref("products").once("value")
      .then(snapshot => {
        ...
        // تحديث عدد الأقسام
        document.getElementById("category-count").innerText = categoryNames.length;
  
        // عرض أسماء الأقسام
        const list = document.getElementById("category-list");
        list.innerHTML = ""; // نفضي القائمة الأول
        ...
        categoryNames.forEach(cat => {
          ...
          li.innerHTML = `
            ${cat}
            <span class="badge bg-primary rounded-pill">${Object.keys(categories[cat] || {}).length} منتج</span>
          `;
          list.appendChild(li);
        });
      })
